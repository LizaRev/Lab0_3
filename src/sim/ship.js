...
export class Ship extends Entity {
  ...
  update(dt, inputs) {
    ...
    this.vel.x *= Math.pow(drag, dt * 60);
    this.vel.y *= Math.pow(drag, dt * 60);
  ...
  fire() {
    ...
    const bulletX =
      this.pos.x + direction.x * (this.radius + 4);

    const bulletY =
      this.pos.y + direction.y * (this.radius + 4);

    const bulletSpeed = 500;

    const bulletVx =
      this.vel.x + direction.x * bulletSpeed;

    const bulletVy =
      this.vel.y + direction.y * bulletSpeed;
